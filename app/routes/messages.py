...
@router.post(
    ...
    description="Sends a new transaction to the SQS FIFO queue based on the data provided in the request body.",
...
@router.post(
    ...
    description="Sends a new transaction to the SQS Dead-Letter Queue (DLQ) based on the data provided in the request body.",
    response_model=MessageSchema,
    dependencies=[Depends(auth_middleware)],
    responses={
        422: {"model": ValidationMessageErrorResponse, "description": "Invalid request payload."},
...
@router.post(
    ...
    description="Reprocesses messages from the Dead-Letter Queue (DLQ) back to the main SQS queue "
                "specified by the queue name. Returns a success message when all messages are reprocessed.",
